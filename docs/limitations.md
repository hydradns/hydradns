...
- **Query logs store client IPs as-is by default.** Per-device DNS activity is visible in the
  ...
  can see which device on your network looked up which domain. That's by design, since
  per-device visibility is the point of a home/office DNS firewall's query log. *Workaround:* set
...
- **Settings page has no backend.** The dashboard has a Settings screen, but the control
  plane has no `/settings` endpoint, so the page can't actually persist anything yet.
